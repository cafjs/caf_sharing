 ...
 *
 * @param {caf.refMap} writerRef A reference to a writable Shared Map.
 *
 * @memberof! module:caf_sharing/ReliableChannel
 * @alias init
 */
 ...
 *
 * @param {caf.refMap} readerRef A reference to a writable Shared Map for
 * acks.
 * @param {string} channelName The name of the channel.
 *
 ...
 *
 *  The return type 'caf.messages' is:
 *
 *       {index: number, messages: Array.<caf.json>}
 *
 *  where the `index` field corresponds to the first message in `messages` or
 * UNKNOWN_ACK_INDEX if no messages.
 *
 * @param {caf.refMap} writerRef A writable map ack counters.
 * @param {caf.refMap} readerRef A read-only map with messages.
 * @param {string} channelName The name of the channel to receive messages from.
 *
 * @return {caf.messages} Messages received in the channel that have not been
